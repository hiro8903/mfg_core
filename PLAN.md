...
### Step 1: 開発環境の基礎固め
- [ ] `rails new`直後の初期状態を **Git** にコミットする。
- [ ] テーブル構成を自動でDBML（設計図）として出力・管理するツールの導入。

### Step 2: 認証とユーザーマスタ (Authentication)
- [ ] `ユーザーマスタ` の Migration とモデル作成。
- [ ] 「ユーザーを登録し、その作業員としてログインできる機能（なりきり操作）」の構築。
